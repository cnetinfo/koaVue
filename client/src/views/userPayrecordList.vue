<template>
  <div>
    <group>
      <cell class="font-size-normal" :title="item.account" v-for="(item,index) in dataList" :key="item.id"  :inline-desc="item.paymentTime" :value="(item.paymentMethod==1?'现金':'网付')+':'+item.paymentmoney+'元'"></cell>
    </group>
...
<script>
  ...
  export default {
    ...
    methods: {
      getList(){
        var keyword = this.$route.query.key||'undefined'
        this.loading = true;
        this.pending = true;
        let url = "getpaymentrecord";
        let param = {
          keyword:keyword,
          page: this.pageIndex
        };
